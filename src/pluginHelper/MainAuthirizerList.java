...
public class MainAuthirizerList
{
	...
	public MainAuthirizerList(String listPath)
	{
		...
		try
		{
			...
			{
				Element x = rootElement.getChild(Text_OP);
				if (x != null)
				{
					if (!x.getText().isEmpty())
					{
	...
	protected void save()
	{
		rootElement.getChild(Text_SOP).setText(Long.toString(SOP));
	...
	protected boolean setOP(long num)
	{
		if (SOP == num)
			return false;
		boolean re = OP.add(num);
		save();
		return re;
	}

	protected boolean deleteOP(long num)
	{
		boolean re = OP.remove(num);
		save();
		return re;
	}
}
